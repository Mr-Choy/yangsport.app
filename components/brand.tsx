...
export function YLogo({ size = 28, gradient = false, color = '#6B6B6B' }: {
  size?: number; gradient?: boolean; color?: string;
}) {
...
export function YMini({ size = 18, color = 'currentColor' }: { size?: number; color?: string }) {
  return (
...
export function ChevronStamp({ size = 20, filled = false }: { size?: number; filled?: boolean }) {
  const gid = useId().replace(/:/g, '');
  if (filled) {
    return (
