...
function App({ tweaks, setTweak }) {
  ...
  const [stringMap, setStringMap] = React.useState(STRINGING_HISTORY);
  ...
  const handleRecordStringing = (customerId, data) => {
    setStringMap(prev => {
      ...
      return { ...prev, [customerId]: [newS, ...(prev[customerId] || [])] };
    });
    // also update last_visit
    setCustomers(prev => prev.map(c =>
      c.id === customerId ? { ...c, lastVisitDays: 0 } : c
    ));
    showToast({
      type: 'redeem',
      title: strings.stringingSaved,
      sub: `${data.racketModel} · ${data.tension} ${strings.lbsUnit}`,
    });
  };
