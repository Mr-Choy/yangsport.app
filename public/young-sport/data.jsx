...
// ─────────────────────────────────────────────────────────────
// ARSENAL: equipment presets + stringing services seed data
// ─────────────────────────────────────────────────────────────
...
const TENSION_PRESETS = [22, 24, 25, 26, 27, 28, 30];

// Stringing services keyed by customer id
const STRINGING_HISTORY = {
  c1: [
    // Aaron Tan — power player, 2 rackets, climbing tension
  ...
  c3: [
    // Wei Ming — single racket, due for restring
...
const FATIGUE = { healthy: 45, aging: 75, fatigue: 90 };

function stringStatus(daysAgo) {
  if (daysAgo < FATIGUE.healthy) return 'healthy';
  if (daysAgo < FATIGUE.fatigue) return 'aging';
  return 'fatigue';
}

// Aggregate: rackets owned (deduped by brand+model, with latest service)
function getRackets(services) {
  const map = new Map();
  // services should come newest-first
  const sorted = [...services].sort((a, b) => a.daysAgo - b.daysAgo);
  for (const s of sorted) {
    const key = `${s.racketBrand}|${s.racketModel}`;
    if (!map.has(key)) {
      map.set(key, {
        brand: s.racketBrand, model: s.racketModel,
        latest: s,
        services: [s],
      });
    } else {
...
function getStringUsage(services) {
  const map = new Map();
  for (const s of services) {
    const key = `${s.stringBrand}|${s.stringModel}`;
    if (!map.has(key)) {
      map.set(key, { brand: s.stringBrand, model: s.stringModel, count: 0 });
    }
    map.get(key).count++;
  }
...
// Tension trend (sorted oldest → newest)
function getTensionTrend(services) {
  return [...services].sort((a, b) => b.daysAgo - a.daysAgo);
}

Object.assign(window, {
  I18N, REWARDS, SEED_CUSTOMERS, SEED_CAMPAIGNS, VISITS_7D, TX_HISTORY,
  EQUIPMENT_PRESETS, TENSION_PRESETS, STRINGING_HISTORY, FATIGUE,
  stringStatus, getRackets, getStringUsage, getTensionTrend,
