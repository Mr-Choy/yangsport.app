...
// Customer Detail
// ─────────────────────────────────────────────────────────────
function ScreenCustomerDetail({ t, lang, customer, idx, transactions, stringingServices, onBack, onAddStamp, onRedeem, onRecordStringing, onToast }) {
  const [newIndex, setNewIndex] = React.useState(-1);
  const [tab, setTab] = React.useState('loyalty'); // 'loyalty' | 'arsenal'
  const c = customer;
  ...
  // build a merged history that includes stringing services
  const stringingTx = (stringingServices || []).map(s => ({
    id: `tx-string-${s.id}`,
    type: 'stringing',
    daysAgo: s.daysAgo,
  ...
  const mergedHistory = [...transactions, ...stringingTx].sort((a, b) => a.daysAgo - b.daysAgo);
    ...
    <div className="ys-content ys-pad-top ys-pad-btm">
      ...
      <div className="det-tabs">
        ...
        <button className={`det-tab ${tab === 'arsenal' ? 'is-active' : ''}`} onClick={() => setTab('arsenal')}>
          ...
          {stringingServices && stringingServices.length > 0 && (
            <span style={{
      ...
      {tab === 'loyalty' ? (
        <LoyaltyTab c={c} t={t} lang={lang} idx={idx}
                    threshold={threshold} cols={cols} newIndex={newIndex}
                    next={next} redeemables={redeemables}
                    history={mergedHistory}
                    onAddStamp={handleAdd}
                    onRedeem={onRedeem}
                    onRecordStringing={() => onRecordStringing && onRecordStringing(c.id)} />
      ) : (
        <ArsenalTab c={c} t={t} lang={lang}
                    services={stringingServices || []}
                    onRecord={() => onRecordStringing && onRecordStringing(c.id)} />
      )}
    </div>
  );
}
...
// ─── Loyalty tab body ─────────────────────────────────────────
function LoyaltyTab({ c, t, lang, idx, threshold, cols, newIndex, next, redeemables, history, onAddStamp, onRedeem, onRecordStringing }) {
  return (
    <>
      {/* Stamp progress hero */}
      ...
      {/* Secondary action: log stringing service */}
      ...
      {/* History (merged: stamps + redeems + stringing) */}
      <div className="sect-row">
...
// ─── Arsenal tab body ─────────────────────────────────────────
function ArsenalTab({ c, t, lang, services, onRecord }) {
  return (
    <>
...
// ─── Unified history row (handles earn / redeem / stringing) ──
function HistoryRow({ tx, t, lang }) {
...
// ─────────────────────────────────────────────────────────────
function AddStampSheet({ open, t, lang, customers, onClose, onStamp, onCreate, initialPhone = '' }) {
  ...
  React.useEffect(() => {
    if (!open) {
      setPhone(initialPhone); setStep('input'); setMatch(null); setName('');
    }
  }, [open, initialPhone]);
